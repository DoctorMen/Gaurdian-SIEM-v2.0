    ...
    <div class="report-container">
        ...
        <div class="report-header">
            <div style="display:flex;justify-content:space-between;align-items:start;">
                <div>
                    ...
                    <span class="severity-badge">{esc(incident.severity)}</span>
                </div>
            ...
            <div class="meta">
                <div><strong>Incident ID:</strong> {esc(incident.incident_id)}</div>
                <div><strong>Classification:</strong> {esc(incident.classification)}</div>
                <div><strong>Generated:</strong> {incident.created_at.strftime('%B %d, %Y %H:%M UTC')}</div>
                <div><strong>Analyst:</strong> {esc(incident.analyst)}</div>
            </div>
        </div>
        ...
        <div class="footer">
            ...
            <p>Generated {incident.created_at.strftime('%Y-%m-%d %H:%M:%S')} |
               Report ID: {esc(incident.incident_id)} |
               Classification: {esc(incident.classification)}</p>
        ...
        if users:
            ...
            if compromised:
                user_text = (f" Compromised accounts include: "
                             f"<strong>{', '.join(esc(u) for u in list(compromised)[:3])}</strong>.")

        return f"""<p>
            Guardian SIEM detected a <strong>{esc(incident.severity.lower())}-severity {esc(threat_type)}</strong>
    ...
    def _render_timeline(self, incident):
        ...
        for event in display_events:
            ...
            <div class="timeline-event">
                <div class="time">{esc(str(ts))}</div>
                <div class="content">
                    <span class="sev-tag tag-{esc(sev)}">{esc(sev)}</span>
                    <strong> [{esc(source)}]</strong>
                    {f'<span style="color:#666;"> ({esc(mitre)})</span>' if mitre else ""}
                    {f'<span style="color:#388e3c;"> Rule: {esc(rule)}</span>' if rule else ""}
                    <br><span style="font-size:13px;color:#555;">{esc(msg)}</span>
                </div>
            </div>""")
    ...
    def _render_iocs(self, incident):
        ...
        if incident.iocs["ip_addresses"]:
            ...
            for ip in sorted(incident.iocs["ip_addresses"]):
                ...
                geo_text = f" ({esc(geo['country'])}, {esc(geo['city'])})" if geo.get("country") else ""
                ips_html += f'<span class="ioc-tag">{esc(ip)}{geo_text}</span>\n'
            sections.append(f'<h3>IP Addresses ({len(incident.iocs["ip_addresses"])})</h3>\n{ips_html}')

        if incident.iocs["domains"]:
            domains_html = "\n".join(f'<span class="ioc-tag">{esc(d)}</span>'
        ...
        if incident.iocs["file_paths"]:
            paths_html = "\n".join(f'<span class="ioc-tag">{esc(p)}</span>'
